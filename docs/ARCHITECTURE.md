...
- **SwiftData never touches CloudKit**: `AppModel` pins `cloudKitDatabase: .none`. SwiftData defaults to `.automatic`, which would mirror the whole local store to the user's private database as soon as the iCloud entitlement is present: the opposite of the local-first promise, and it refuses to load the store at all because the models have non-optional attributes without defaults. The battle models themselves compile in every configuration so the schema never diverges between battles-on and battles-off builds.
