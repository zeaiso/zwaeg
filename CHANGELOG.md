...
- Recipes translate themselves. Bundled batch translations
  (scripts/translate_recipes.py generates them for every app language) show
  instantly with an original/translation toggle. Languages without a
  bundled table fall back to Apple's on-device translation — and where
  Apple can't produce the app language (Danish, Swedish, ...), an English
  translation is offered instead. Nothing leaves the device either way.
